  ...
  render() {
    ...
    if (_.size(userTraits) || _.size(accountLinks)) {
      const traits = userTraits.map(user => {
        const account = _.get(
          _.find(accountLinks, acc =>
            _.isEqual(acc.userIdentity, user.userIdentity)
          ),
          "accountIdentity"
        );
        return `
    ...
    if (events.length) {
      const eventString = _.map(events, e => {
        const identity = JSON.stringify(
          e.userIdentity || e.accountIdentity,
          null,
          2
        );
        return `${
          e.userIdentity ? "User " : e.accountIdentity ? "Account " : ""
        }Identity:
${identity}
Event:
// Name
${e.event.eventName}
${
          !_.isEmpty(e.event.properties)
            ? `// Properties
${JSON.stringify(e.event.properties, null, 2)}`
            : ""
        }
${
          !_.isEmpty(e.event.context)
            ? `// Context
